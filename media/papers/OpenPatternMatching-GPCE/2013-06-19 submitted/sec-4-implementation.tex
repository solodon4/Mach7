...
type of the subject \code{S}. Patterns like wildcard pattern will leave the \code{S}
...
fibbonaci functions that involve generalized n+k patterns like \code{2*m+1} and 
\code{2*m} to distinguish and decompose odd and even numbers respectively.
...
\code{Cart} layout to be the default. We then define bindings for each 
...
\code{e_ii}. Finally, the library generates the code that sequentially applies 
