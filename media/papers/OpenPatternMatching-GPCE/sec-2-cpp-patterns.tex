...
The object analyzed through pattern matching is commonly called the \term{scrutinee} 
or \term{subject}, while its static type is commonly called the \subterm{type}{subject type}.
...
\noindent
The subject \code{n} is passed as an argument to the \code{Match} statement and is 
then analyzed through \code{Case} clauses that list various patterns. In the 
...
patterns}. The list of primitive patterns is often extended with a \subterm{pattern}{predicate 
pattern} (e.g. as seen in Scheme~\cite{Wright95}), which allows the use of any 
unary predicate or nullary member-predicate as a pattern: e.g. \code{Case(even) 
...} (assuming \code{bool even(int);}) or \code{Case([](int m) \{ return m^m-1; \}) ...} 
...
\emph{accepted} or \emph{rejected}. Guards gives rise to \subterm{pattern}{guard 
patterns}, which in \emph{Mach7} are expressions of the form $P$\code{|=}$E$, where $P$ is a 
...
%% a two-argument case clause taking a variable and guard patterns to check that 
...
Pattern matching is closely related to \term{algebraic data types}. In ML and Haskell, an 
\term{Algebraic Data Type} is a data type each of whose values are picked from a 
disjoint sum of data types, called \term{variants}. Each variant is a \term{product 
...
expression in the pattern-matching context is called a \subterm{pattern}{constructor pattern} 
...
which take the form \code{C<Ti>(}$P_1,...,P_{m_i}$\code{)}, where $T_i$ is the name of 
the user-defined type we are decomposing and $P_1,...,P_{m_i}$ are patterns that 
will be matched against members of $T_i$. `\code{C}' was chosen to abbreviate 
...
In general, a \term{pattern combinator} is an operation on patterns to produce a 
...
%which makes them particularly expressive. 
...
Chris~Okasaki~\cite[\textsection 3.3]{okasaki1999purely} implemented in 
\emph{Mach7}:
...
variable pattern using the conjunction combinator: \code{a && any(...)}.
