...
data variants may form subtyping relationships between themselves, which made us 
...
manuscript for a discussion of the approaches~\cite{fool12}. Interestingly, most of 
...
\noindent
The corresponding C++ data types would most likely be parameterized, but for now 
we will just use simple classes, duplicating the body:
...
\noindent
Imagine, however, that in addition to evaluation we would like to also provide a pretty 
...
Our solution is a proof of concept that sets a minimum threshold for the
performance, brevity, clarity and usefulness of a language solution for pattern 
matching in C++. It provides full functionality, so we can experiment with the use 
...
Due to page restrictions, this paper will only deal with the part of 
...
efficiency in comparison to the visitor design pattern might become a major factor 
...
With the help of our library, the function \code{eval} from the above example or any 
...
access to the properly-casted derived type via the \code{matched} object. The case 
clauses would thus look like this:
...
in section~\ref{sec:eval}, it runs about as fast as the OCaml version and up to 
80\% faster (depending on the usage scenario, compiler and underlying hardware) 
than handcrafted C++ code based on the \emph{visitor design pattern}.
...
facilities that depend on the dynamic types of objects. A practical benefit of our 
solution is that it can be used right away with any compiler with a descent 
support of C++0x without requiring the installation of any additional tools or 
preprocessors.
