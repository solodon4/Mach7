...
cases in which optimal $H_{kl}^V$ had only one ``extra'' vtbl-pointer ($m=1$). 2.63\% 
experiments had $H_{kl}^V$ with 2 conflicts, 0.87\% with 3 etc. 

It is important to understand that these numbers do not indicate that the hash 
function we used is better than other hash functions, they do indicate instead 
that the set of vtbl-pointers present in many applications is particularly 
suitable for such a hash function.
