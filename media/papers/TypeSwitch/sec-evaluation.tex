...
To give a broader view on how both techniques compare under different 
...
in regular font (e.g. \f{67}), indicate when type switching was faster than 
visitors, while underlined numbers in bold (e.g. \s{18}), indicate when visitors 
...
\begin{figure}[htbp]
  ...
  \caption{Performance comparison with OCaml and Haskell code}
...
\end{tabular}
\caption{Percentage of subhierarchies rendering a given number of conflicts ($m$) under different size constraints}
\label{fig:size}
\end{figure}
...
Pivot\cite{Pivot09} using Mach7. Pivot's class hierarchy 
...
needed them to come after. Redundancy checking support provided by Mach7 
...
The \textoverline{blue} part of each bar corresponds to the memory used by cache, while the 
\textoverline{maroon} -- to the memory used by the hash table. The ratio of the \textoverline{non-opaque}
part of both \textoverline{colors} to the entire part of that \textoverline{color} indicates the load factors 
