...
\titlebanner{Draft rejected from PLDI 2012} % These are ignored unless
...
object-oriented and functional programming techniques. We present techniques
for implementing efficient type switching, type testing, pattern matching,
predicate dispatch, and multi-methods. The techniques are general and cope
well with C++ multiple inheritance.
 
Our library-only implementation provides a pattern-matching notation. It
...
equivalent code in languages with built-in type switching constructs, such as
OCaml. The pattern-based library code is easier to use and more expressive
than hand-coded visitors. The library is non-intrusive and does not have
extensibility restrictions. It also avoids control inversion.
 
The library was motivated by applications involving large, typed, abstract
syntax trees. Being a library-only solution allows us to use production
quality compilers and tool chains.
