...
\emph{polymorphic encoding}) or a dedicated member of a base class (referred to 
as \emph{tagged encoding}).
...
implementatin technique (e.g. virtual tables) for virtual function dispatch.
However, in practice, the vast majority of C++ compilers use well-established
technologies \cite{C++ABI}.  The technique presented in this paper
works with any C++ compiler that follows that C++ ABI. Microsoft's own ABI
is not publically available and thus we cannot formally verify that it
satisfies our requirements. Nevertheless, we did run numerous experiments
with various 
class hierarchies and have sufficient confidence that our approach can be used 
in Visual C++. We thus include experimental results for this compiler as well.
...
\emph{polymorphic class}~\cite[\textsection 10.3]{C++0x}. The C++ ABI defines 
...
pointing to a virtual table. A \emph{virtual table} (v-table) is a table of information used 
...
procedure for classes described in \textsection2.4 of the C++ ABI~\cite{C++ABI} 
requires dynamic classes either to allocate vtbl-pointer at offset 0 or share 
one from its primary base class, which is by definition at 
