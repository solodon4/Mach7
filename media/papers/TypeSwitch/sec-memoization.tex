...
Relying on \code{dynamic_cast} also makes an implicit semantic choice that we 
...
More often than not, however, we will be interesting in obtaining a reference to 
...
subobjects, but there seem to be no easy way of identifying them given just an 
object descriptor.
...
Under non-virtual inheritance members of the base class are typically layed out 
...
classes). CFront, the first C++ compiler, puts the vtbl-pointer 
...
Linux versions of Intel and HP compilers, and compilers from ARM.} % Phrase from http://morpher.com/documentation/articles/abi/  
...
for this discussion, it is important to realize that every (sub)object of a 
...
located, but because of layout independence such an offset is going to be the 
same within any subobject of effective type \code{S}. For a library 
implementation we assume presence of a function 
...
Each of the \code{vtbl} fields shown in 
