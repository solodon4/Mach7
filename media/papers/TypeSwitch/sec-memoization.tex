...
\Cpp{} does not have direct support of algebraic data types, but they can be 
...
The \Cpp{} standard requires that information about types be available at run time 
...
its argument \code{Ti} and a constant $i$, which can be obtained from 
...
until run-time, their offset from the module's base address is. This can often be 
...
Probabilities $p_i$ can be estimated in a compiler setting through profiling, 
