...
directly through subject's \code{taglist} data member. The array is of 
...
We also assume the tag-constant associated with a class \code{Di} be accessible 
through a static constant \code{Di::class_tag}. These simplifications are not 
...
can again be implemented with a case clause on subject type's tag: \code{case S::class_tag:}
...
the tags herself. The use of static cast %to obtain proper reference once the most specialized derived class has been established, 
