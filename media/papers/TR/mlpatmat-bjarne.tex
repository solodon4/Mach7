...
values accepted by the pattern. A pattern represents a predicate on 
...
found its way into many imperative programming languages like 
...
Eiffel\cite{Moreau:2003} and others. It is relatively
easy to provide pattern matching when designing a new language, but
to introduce it into a language in widespread use is a 
challenge. The obvious utility of the feature may be compromised by
the need to fit into the language's syntax, semantics, and tool chains.
A prototype implementation will require more effort than for an experimental language,
but will be harder to get into use because mainstream users are unwilling
...
Our current solution is a proof of  
concept that sets a minimum threshold for performance, brevity, clarity and 
usefulness of a language solution for pattern matching in C++. It provides
full functionality, so that users can experiment with 
pattern matching in C++ and us experiment with a language solution.
To give an idea of what our library offers, look at an example from 
a domain where pattern matching is considered to provide terseness and 
...
Describe unified syntax as well as specialized syntax
...
the second is based on \emph{decision trees} and is attributed to ???\cite{}. 
...
Our approach can be combined with either backtracking or decision tree approaches. 
