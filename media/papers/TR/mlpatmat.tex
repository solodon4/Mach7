...
with it will certainly diminish the conveniene of having just a switch 
statement. Besides, forwarding overhead might loose some of the performance 
...
\Rule{match statement}     & $M$       & \is{}  & \code{Match(}$e$\code{)} $\left[C s^*\right]^*\left[O s^*\right]$ \code{EndMatch} \\
\Rule{case clause}         & $C$       & \is{}  & \code{Case(}$T\left[,x\right]^*$\code{)} \\
                           &           & \Alt{} & \code{Que(} $T\left[,\omega\right]^*$\code{)} \\
\Rule{default clause}      & $O$       & \is{}  & \code{Otherwise(}$\left[,x\right]^*$\code{)} \\
...
\Rule{guard pattern}       & $\varrho$ & \is{}  & $\eta \models \xi$ \Alt{} $\mu \models \xi$ \\
...
dedicate \code{EndMatch} macro, to properly close the syntactic structure 
introduced with \code{Match} and followed by \code{Case},\code{Que} and 
\code{Otherwise} macros. Examples of match statement has already been presented 
in \textsection\ref{sec:intro} and \textsection\ref{sec:xmpl}.

We support two kinds of {\bf case clauses}, both of which take a target 
expression as their first argument. \code{Case} clause can only be used for 
simple patterns. Its main advantage is implicit introduction of all the 
variables into the clause's scope, bound to corresponding members. \code{Que} 
clause permits nested patterns as its arguments, but naturally requires all the 
variables used in the patterns to be explicitly pre-declared.

An optional {\bf default clause} provides handling for cases that were rejected 
by case clauses. The clause is not required to be the last clause of the match 
statement and will have the same semantics elsewhere when used with \emph{tagged  
class} and \emph{discriminated union} encodings. With \emph{polymorphic base 
class}, however, it will effectively hide all subsequent case clauses, which is 
why we suggest it to always be placed last (hence the choice of name -- 
otherwise). Default clause may take optional variable patterns, in which case it 
behaves in exactly the same way as \code{Case} clause whose target type is 
selector type.

{\bf Target expression} used by the case clauses can either be a target type, 
...
original combination of target type and layout. Views are discussed in greater 
details in~\textsection\ref{sec:view}.
...
respectively. This transformation is further explained in~\textsection\ref{sec:sem} 
...
\textsection\ref{sec:bg} in C++ enhenced with our SELL:
...
use of any variables bound by preceeding argument positions. Naturally, a guard 
...
of fast fibbonaci algorithm taken almost verbatim from the book. Function 
...
int fib(int n)
{
    ...
    if (match<int>(2*m)(n))   return sqr(fib(m+1)) - sqr(fib(m-1));
    if (match<int>(2*m+1)(n)) return sqr(fib(m+1)) + sqr(fib(m));
}
\end{lstlisting}
...
into a variable. The position is ignored alltogether and the pattern matching 
continues.
...
variable of type \code{T} can. In few cases where this doesn't happen 
...
of bindings. We discuss layouts in greater details in \textsection\ref{sec:bnd}.
...
of the above that restricted to only expression computable at compile time. {\bf 
...
{\Gamma \vdash e \evalspp v,\Gamma_0 \\ \Gamma_0 \vdash \DynCast{\tau}{v} \evalspp u^{\tau},\Gamma_1 \\\\
 \Gamma_1    \vdash \Delta_1    ^{\tau,l}(u) \evalspp v_1,    \Gamma_1'     \\ \Gamma_1'    \stackrel{flt}{\vdash} \omega_1     \evals \pi_1,     \Gamma_1''     \\ \Gamma_1''    \vdash \pi_1(v_1)         \evals true, \Gamma_2     \\\\
 \Gamma_2    \vdash \Delta_2    ^{\tau,l}(u) \evalspp v_2,    \Gamma_2'     \\ \Gamma_2'    \stackrel{flt}{\vdash} \omega_2     \evals \pi_2,     \Gamma_2''     \\ \Gamma_2''    \vdash \pi_2(v_2)         \evals true, \Gamma_3     \\\\
 \cdots \\\\
 \Gamma_{i-1}\vdash \Delta_{i-1}^{\tau,l}(u) \evalspp v_{i-1},\Gamma_{i-1}' \\ \Gamma_{i-1}'\stackrel{flt}{\vdash} \omega_{i-1} \evals \pi_{i-1}, \Gamma_{i-1}'' \\ \Gamma_{i-1}''\vdash \pi_{i-1}(v_{i-1}) \evals true, \Gamma_i     \\\\
 \Gamma_i    \vdash \Delta_i    ^{\tau,l}(u) \evalspp v_i,    \Gamma_i'     \\ \Gamma_i'    \stackrel{flt}{\vdash} \omega_i     \evals \pi_i,     \Gamma_i''     \\ \Gamma_i''    \vdash \pi_i(v_i)         \evals false,\Gamma_{i+1} \\\\
}
{\Gamma\vdash ($match$\langle\tau\left[,l\right]\rangle(\omega_1,...,\omega_k))(e) \evals \nullptr{},\Gamma_{i+1}}
\end{mathpar}
...
{\Gamma \vdash e \evalspp v,\Gamma_0 \\ \Gamma_0 \vdash \DynCast{\tau}{v} \evalspp u^{\tau},\Gamma_1 \\\\
 ...
 \Gamma_2    \vdash \Delta_2    ^{\tau,l}(u) \evalspp v_2,    \Gamma_2'     \\ \Gamma_2'    \stackrel{flt}{\vdash} \omega_2     \evals \pi_2,     \Gamma_2''     \\ \Gamma_2''    \vdash \pi_2(v_2)         \evals true, \Gamma_3     \\\\
 \cdots \\\\
%\Gamma_{k-1}\vdash \Delta_{k-1}^{\tau,l}(u) \evalspp v_{k-1},\Gamma_{k-1}' \\ \Gamma_{k-1}'\stackrel{flt}{\vdash} \omega_{k-1} \evals \pi_{k-1}, \Gamma_{k-1}'' \\ \Gamma_{k-1}''\vdash \pi_{k-1}(v_{k-1}) \evals true, \Gamma_k     \\\\
 \Gamma_k    \vdash \Delta_k    ^{\tau,l}(u) \evalspp v_k,    \Gamma_k'     \\ \Gamma_k'    \stackrel{flt}{\vdash} \omega_k     \evals \pi_k,     \Gamma_k''     \\ \Gamma_k''    \vdash \pi_k(v_k)         \evals true, \Gamma_{k+1} \\\\
}
...
be casted to $\tau$. If it does, matching succeds, binding variable to the 
result of \code{dynamic_cast}. If it doesn't, matching fails 
(\RefTirName{Variable}).
...
\item Presense of a unique solution should be indicated with acceptance of the 
...
\begin{lstlisting}
@$\Psi_f^\tau:\tau_r\times\tau_1\times\cdots\times1\times\cdots\times\tau_k\rightarrow bool\times\tau$@
\end{lstlisting} 

\noindent
such that $\Psi_f^\tau[r](c_1,\cdots,\bullet,\cdots,c_k)$ for a given function 

\begin{lstlisting}
@$f:\tau_1,\cdots,\tau,\cdots,\tau_k\rightarrow\tau_r$@
\end{lstlisting} 

\noindent should return a pair composed of a boolean indicating acceptance of a 
pattern $f(c_1,\cdots,x^\tau,\cdots,c_k)=r$ and a solution with respect to $x$ 
...
own operation $f$ in \textsection\ref{}.
...
to the tree pattern can be smaller than the number of binding positions defined 
...
gets accepted we move on to matching the next argument (\RefTirName{Tree-False}). 
...
patterns}(\RefTirName{Tree-True}) while maintaining the composibility with the 
rest of the library.

Our second rule set deals with semantics of a \emph{match statement}.

\begin{mathpar}
\inferrule[Match]
{\Gamma \vdash e \evalspp v,\Gamma_1 \\
 \Gamma_1    \vdash_v ‘_1    \evals false,\Gamma_2 \\
 \Gamma_2    \vdash_v ‘_2    \evals false,\Gamma_3 \\
 \cdots \\
 \Gamma_{i-1}\vdash_v ‘_{i-1}\evals false,\Gamma_i \\
 \Gamma_i    \vdash_v ‘_i    \evals true, \Gamma_{i+1} \\
 \Gamma_{i+1}\vdash_v s_{ij}^* \evalspp u,\Gamma'
}
{\Gamma\vdash Match(e) \left[C_i s_{ij}^*\right]^*_{i=1..n}\left[O s_j^*\right] EndMatch \evals u,\Gamma'}
\end{mathpar}

\begin{mathpar}
\inferrule[Que-Type]
{\Gamma \vdash $match$\langle\tau\rangle(\vec{\omega})(v) \evals b,\Gamma'}
{\Gamma \vdash_v Que(\tau,\vec{\omega}) \evals b,\Gamma'}

\inferrule[Que-Layout]
{\Gamma \vdash $match$\langle\tau,l\rangle(\vec{\omega})(v) \evals b,\Gamma'}
{\Gamma \vdash_{v^\tau} Que(l,\vec{\omega}) \evals b,\Gamma'}
\end{mathpar}

Our second rule set deals with evaluation of expression templates reffered to 
from the previous rule set via $\stackrel{eval}{\vdash}$. The judgements are of 
...
separate \emph{construction} of objects with \emph{constructors} and their 
decomposition through pattern matching with \emph{deconstructors}.
...
The following grammar defines a syntax for a sublange a user will use to specify 
decomposition of various classes for pattern matching:\footnote{We reuse several 
meta-variables introduced in the previous grammar}
...
\Rule{kind value}              & $kv$      & \is{}  & \code{KV(}$q$\code{);}   \\
\Rule{template-parameter-list} & $\vec{p}$ &        & C++\cite[\textsection A.12]{C++0x} \\
\Rule{qualified-id}            & $q$       &        & C++\cite[\textsection A.4]{C++0x} \\
\end{tabular}
\end{center}

User provides decomposition of a given class by specializing a traits-like class 
\code{bindings<T}$\left[,l\right]$\code{>}. For example the following definition 
provides bindings to a standard library type \code{std::complex}:
...
The above definition states that when pattern matching against 
\code{std::complex<T>} for any given type \code{T}, use the result of invoking 
member-function \code{real()} to obtain the value for the first pattern matching 
position and \code{imag()} for the second position. \code{CM} stands for 
\emph{Class Member} and generates an unspecified code understood by the rest of 
the library that maps position specified as its first argument to a member, 
specified as its second argument. \code{CM} will accept the following entities 
as its second parameter:

\begin{itemize}
\item Pointer to a data member of the target type
\item Pointer to a nullary member-function of the target type
\item Pointer to a unary external function taking target type by pointer 
      reference or value.
\end{itemize}

\code{CM} will unambiguosly pick one of the above when several overloaded 
members with that name exist of different arity e.g. 
\code{T std::complex<T>::real() const} will be prefered to \\
\code{void std::complex<T>::real(T)}.

Note that binding definitions made this way are \emph{non-intrusive} since the 
original class definition is not touched and respect \emph{encapsulation} since 
only the public members of the target type will be accessible from within 
\code{bindings} specialization.

Note also that the above definition is made once for all instantiations of 
\code{std::complex}, but can be fully or partially specialized for cases of 
interest. Non-parameterized classes should fully specialize the \code{bindings} 
trait to define their own bindings.

The above definition will fall short, however, when we have to define bindings 
for an algebraic data type encoded with discriminated union. In this case we 
have a single target type and multiple bindings associated with it. To account for this, we recognize that a class (whether discriminated union or not) 
...
its kind. Consider the following discriminated union describing various shapes:
...
Similarly to \code{CM}, the \code{KS} macro inside defult bindings for 
\code{ADTShape} tells the library which member should be used to access 
\emph{kind selector} for the type. Correspondingly each of the layouts uses 
\code{KV} macro to indicate \emph{kind value} -- a value of the \emph{kind 
selector} this layout is specified for. Note that the second parameter used to 
distinguish this specialization from default one matches the value specified 
with \code{KV} as we require. To understand the difference between the two, 
think of a tagged class encoding, which will use \code{KV} in all derived 
classes to indicate kind value that uniquely identifies them, but will not have 
to provide the layout parameter as their class already uniquely identifies 
target type\footnote{Unless, of course, one would likes to specify multiple 
layouts per that target class}. Bindings for the base class in this case will 
similarly have to use \code{KS} macro to indicate which field should be used as 
a kind selector.
