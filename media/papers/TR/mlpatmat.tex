...
\Rule{case clause}         & $C$       & \is{}  & \code{Case(}$T\left[,x\right]^*$\code{)} \\
                           &           & \Alt{} & \code{Que(} $T\left[,c|x|\pi\right]^*$\code{)} \\
...
\Rule{tree pattern}        & $\mu$     & \is{}  & \code{match<}$\tau\left[,l\right]$\code{>(}$\left[c|x|\pi\right]^*$\code{)} \\
\Rule{guard pattern}       & $\varrho$ & \is{}  & $\eta \models \varepsilon$ \Alt{} $\mu \models \varepsilon$ \\
...
\Rule{xt expression}       & $\varepsilon$   & \is{}  & $\chi$ \Alt{} $\varepsilon \oplus c$ \Alt{} $c \oplus \varepsilon$ \Alt{} $\ominus \varepsilon$ \Alt{} $(\varepsilon)$ \Alt{} $\varepsilon \oplus \varepsilon$ \\
...
introduced with \code{Match} and followed by \code{Case} and \code{Que} macros.
...
{\bf Target expression} used by the case clauses can either be a target type or 
a constant value, representing a layout. The latter is only allowed for 
discriminated union encoding of algebraic data types, in which case the library 
assumes the target type to be selector type.

{\bf Match expression} can be seen as a short version of match statement with 
only one Que clause. Once a pattern is created, it can be applied to an 
...
library be composable. The metavariable $\pi$ representing {\bf pattern} simply 
summarizes different kinds of \emph{applicative patterns} we support.
...
\begin{mathpar}
\inferrule[Guard]
{\Gamma\vdash e \evalspp v_1,\Gamma_1 \\ \Gamma_1\vdash \pi(v_1) \evals b_2,\Gamma_2 \\ \Gamma_2\stackrel{eval}{\vdash} \varepsilon \evals b_3,\Gamma_3}
{\Gamma\vdash (\pi \models \varepsilon)(e) \evals (b_2 \wedge b_3),\Gamma_3}
\end{mathpar}

\begin{mathpar}
\inferrule[Tree-Nullptr]
{\Gamma \vdash e \evalspp v,\Gamma_0 \\ \Gamma_0 \vdash \DynCast{\tau}{v} \evalspp \nullptr{},\Gamma_1}
{\Gamma\vdash ($match$\langle\tau\rangle(\pi_1,...,\pi_k))(e) \evals \nullptr{},\Gamma_1}
\end{mathpar}

\begin{mathpar}
\inferrule[Tree-False]
{\Gamma \vdash e \evalspp v,\Gamma_0 \\ \Gamma_0 \vdash \DynCast{\tau}{v} \evalspp u^{\tau},\Gamma_1 \\\\
 \Gamma_1    \vdash \Delta_1    ^\tau(v) \evalspp v_1,    \Gamma_1'     \\ \Gamma_1'    \vdash \pi(v_1    ) \evals true, \Gamma_2     \\\\
 \Gamma_2    \vdash \Delta_2    ^\tau(v) \evalspp v_2,    \Gamma_2'     \\ \Gamma_2'    \vdash \pi(v_2    ) \evals true, \Gamma_3     \\\\
 \cdots \\\\
 \Gamma_{i-1}\vdash \Delta_{i-1}^\tau(v) \evalspp v_{i-1},\Gamma_{i-1}' \\ \Gamma_{i-1}'\vdash \pi(v_{i-1}) \evals true, \Gamma_i     \\\\
 \Gamma_i    \vdash \Delta_i    ^\tau(v) \evalspp v_i,    \Gamma_i'     \\ \Gamma_i'    \vdash \pi(v_i    ) \evals false,\Gamma_{i+1} \\\\
}
{\Gamma\vdash ($match$\langle\tau\rangle(\pi_1,...,\pi_k))(e) \evals \nullptr{},\Gamma_{i+1}}
\end{mathpar}
...
{\Gamma \vdash e \evalspp v,\Gamma_0 \\ \Gamma_0 \vdash \DynCast{\tau}{v} \evalspp u^{\tau},\Gamma_1 \\\\
 ...
 \Gamma_2    \vdash \Delta_2    ^\tau(v) \evalspp v_2,    \Gamma_2'     \\ \Gamma_2'    \vdash \pi(v_2    ) \evals true, \Gamma_3     \\\\
 \cdots \\\\
 \Gamma_{k-1}\vdash \Delta_{k-1}^\tau(v) \evalspp v_{k-1},\Gamma_{k-1}' \\ \Gamma_{k-1}'\vdash \pi(v_{k-1}) \evals true, \Gamma_k     \\\\
 \Gamma_k    \vdash \Delta_k    ^\tau(v) \evalspp v_k,    \Gamma_k'     \\ \Gamma_k'    \vdash \pi(v_k    ) \evals true, \Gamma_{k+1} \\\\
}
...
interpretation}\cite{CousotCousot92-1}. In general, \emph{backward collecting 
...
functions whose inverse is unique. An example of such function is Peano curve 
that uniquely maps $(0,1)$ onto $(0,1)^2$. Even when such mappings are 
not one-to-one, their restriction to a given argument often is. Most 
generalizations of n+k patterns seem to agree on the following rules:
...
the form $\Gamma\vdash \varepsilon \evals v,\Gamma'$ that can be interpreted as given an 
environment $\Gamma$, evaluation of an expression template $\varepsilon$ results in value 
...
\inferrule[Unary]
{\Gamma\vdash \varepsilon \evals v,\Gamma_1}
{\Gamma\vdash \ominus \varepsilon \evals \Sem{\ominus} v,\Gamma_1}

\inferrule[Binary]
{\Gamma\vdash \varepsilon_1 \evals v_1,\Gamma_1 \\ \Gamma_1\vdash \varepsilon_2 \evals v_2,\Gamma_2}
{\Gamma\vdash \varepsilon_1 \oplus \varepsilon_2 \evals v_1\Sem{\oplus}v_2,\Gamma_2}

\inferrule[Binary]
{\Gamma\vdash \varepsilon \evals v,\Gamma_1}
{\Gamma\vdash \varepsilon \oplus c \evals v\Sem{\oplus}c,\Gamma_1}

\inferrule[Binary]
{\Gamma\vdash \varepsilon \evals v,\Gamma_1}
{\Gamma\vdash c \oplus \varepsilon \evals c\Sem{\oplus}v,\Gamma_1}
\end{mathpar}
