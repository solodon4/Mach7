...
Wildcard pattern is accepted everywhere where an \emph{xt variable} $x$ or a 
regular variable $v$ are accepted. The important difference from a use of 
an unused variable is that no code is executed to obtain a value for a given 
position and copy that value into a variable. The position is ignored 
alltogether and the pattern matching continues.
...
{\bf xt variable} stands for \emph{expression-template variable} and refers to 
variables whose type is \code{variable<T>} for any given type \code{T}. They are 
terminal symbols in our SELL that let us build various expression templates. 
These variables hold a value of type \code{T} and in most cases can be used in 
the same context where a variable of type \code{T} can. In few cases where this 
doesn't happen implicitly, the user might need to put an explicit cast to 
\code{T&} or \code{const T&}.
...
We use natural semantics\cite{Kahn} (big-step operational semantics) to describe 
our pattern-matching semantics. As with syntax, we do not formalize our 
semantics to the entire language, but concentrate only on presenting relevant 
parts of our extension. We assume the entire state of the program is modeled by 
an environment $\Gamma$, which we can query as $\Gamma(v)$ to get a value of a 
variable $v$. We describe our semantics with several rule sets that deal with 
different parts of our syntax.

Our first rule set deals with pattern application $\pi(e)$, which essentially 
performs matching of pattern $\pi$ against expression $e$. The judgements are of 
...
the form $\Gamma\vdash x \evals v,\Gamma'$ that can be interpreted as given an 
environment $\Gamma$, evaluation of an expression template $x$ results in value 
...
\begin{tabular}{rp{0em}cl}
\Rule{bindings}                &           & \is{}  & $b^*$ \\
\Rule{binding definition}      & $b$       & \is{}  & \code{template <}$\left[\vec{p}\right]$\code{>} \\
