...
programming languages like Pizza\cite{Odersky97pizzainto}, Scala\cite{Scala2nd}, 
Fortress\cite{}, Java\cite{Liu03jmatch:iterable,HydroJ2003}, C++\cite{Prop96}, 
...
constructors expect a value of a carthesian product of two \codeocaml{expr} 
...
cannot extend it with new variants without modifying its defintion. This is not 
...
programs by case analysis and alows the compiler to perform an automatic 
...
never be entered because of preceeding one being more general. Object-oriented 
...
of preceeding definitions. The notable exception in C++ semantics that prefers 
the \emph{first-fit} strategy is ordering of \code{catch} handlers of a 
try-block. Similarly to functional languages the compiler will perform 
...
The patterns that work with algebraic data type we have seen so far are 
generally called \emph{tree patterns}. Pattern matching however is not used 
solely with algebraic data types and can equally well be applied to built-in 
types. The following Haskell code defines factorial function in the form of 
equations:
...
function factorial is 0. The \emph{variable pattern} \codehaskell{n} in the left hand 
side of the second equation will match any value, \emph{binding} variable 
\codehaskell{n} to that value in the right hand side of equation. Similarly to variable 
...
conscies notation such language feature could facilitate automatic proof of 
termination of such functions by compiler. Peano numbers, used as an algebraic 
data type representation of natural numbers, however, are not always the best 
abstraction for representing other mathematical operations. This, together with 
numerous ways of defining semantics of generalized n+k patterns were some of the 
reasons why feature was actually never generalized. Moreover, numerous debates 
over semantics and usefullness of the feature resulted in n+k patterns being 
removed from the language in Haskell 2010 standard\cite{haskell2010}.

While n+k patterns were something only Haskell had, another common feature of 
many programming languages that have pattern matching are guards. A \emph{guard} 
...
languages like Prolog. In particular, it can be viewed as "one-way" matching, 
whereas Prolog allows "two-way" matching (via unification).
