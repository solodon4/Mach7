...
\begin{figure}[htbp]
  ...
  \caption{Type switching based on dynamic\_cast}
  \label{fig:DCastVis1}
\end{figure}
...
classes number 2*N and 2*N+1 derived from a class with number N.
...
\code{memoized_cast} in \textsection\ref{memcast}. As can be seen from the graph 
...
\textsection\ref{memcast}, while we keep it in the chart to give perspective on 
an even faster solution to dynamic casting.
...
overhead will have a poor chance of being adopted. Switching on sequentially 
allocated tags was one of the few techniques that could achieve constant 
overhead, and thus compete and even outperform visitors. Unfortunately the 
scheme has problems of its own that make it unsuitable.
...
23.2.5(13)]{C++0x}. Parameterized over the type T of elements stored in the hash 
table we implemented the above data structure in a reusable class 
\code{vtblmap<T>}.

Depending on the amount of collisions that happens in the cache our vtbl 
...
path of execution. This is we limit our indexing function to shifts and masking 
operations as well as choose the size of cache to be a power of 2.
...
conflicts between the vtbl pointers. The required size, however, can be too big 
to justify the use of memory. This is why in our current implementation we 
always consider only 2 different cache sizes: $2^k$ and $2^{k+1}$ where 
$2^{k-1} \le n \leq 2^k$. This guarantees us that the cache size is never more 
than 4 times bigger than the minimum required cache size.
...
shift the vtbl pointer by to compute cache index, vary further and compute it in 
such a way that minimizes the number of vtbl pointer collisions.
...
framework\cite{Pivot} using our pattern-matching library. We ran the pretty 
...
resulting probability distribution can be seen on a thicker line in 
...
probability has power-law distribution. This is likely to be a specific of our 
application, however the above picture demonstrates that frequency of certain 
...
other nodes. Naturally we would to avoid conflicts on such classes in the cache.
