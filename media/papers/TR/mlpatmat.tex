...
Note that in this case both selector type and target types are the same and 
...
result of application is always \code{bool} except for the tree pattern, where 
it is a value convertible to \code{bool}. The actual value in this case is going 
to be a pointer to target type \code{T} in case of a successful match and a 
...
\noindent
{\bf tree pattern} like the one above takes a type being matched and an optional 
...
constants and regular C++ variables to be passed as an argument of the tree 
pattern, which are treated as \emph{value patterns} and \emph{variable patterns} 
respectively. Tree patterns can be arbitrarily nested, for example:

\begin{lstlisting}[keepspaces,columns=flexible]
double x,y;
if (match<Circle>(match<Point2D>(x,y),4)(shape)) ...
\end{lstlisting}

The above example assumes presense of a class hierarchy of \code{Shape}s and 
checks whether a given \code{shape} is a \code{Circle} with radius 4. If it is, 
the coordinates of its center will be bound to regular variables \code{x} and 
\code{y} of type \code{double}.

\noindent
{\bf guard pattern} in our SELL consist of two expressions separated with 
\code{|=}: an expression being matched (left operand) and a condition (right 
operand). The right operand is allowed to make use of the variable bound in the 
left operand. When used on arguments of a tree pattern, the condition is also 
allowed to make use of any variable bound by preceeding argument positions. 
Naturally, guard pattern that follows a tree pattern may use all the variables 
bound by the tree pattern. For example:
...
variable of a dedicated type \code{wildcard} bearing no state. 
Wildcard pattern is accepted everywhere where an \emph{xt variable} $x$ or a 
regular variable $v$ are accepted.

\noindent
{\bf xt expression} similarly refers to an \emph{expression-template expression} 
built by applying a given operator to argument expression templates. We use this 
syntactic category to distinguish lazily evaluated expressions introduced by our 
SELL from eagerly evaluated expressions, directly supported by C++.
...
described by subsequent categories. These variables hold a value of type 
\code{T} and in most cases can be used in the same context where a variable of 
type \code{T} can. In few cases where this doesn't happen implicitly, the user 
might need to put an explicit cast to \code{T&} or \code{const T&}. 
...
\noindent
{\bf identifier} will only refer to variable names in our SELL, even though it 
has a broader meaning in the C++ grammar.

\noindent
{\bf constant-expression} is a subset of the above that restricted to only 
expression computable at compile time.

\noindent
{\bf expression} subsumes any valid C++ expression. We use expression $e^\tau$ 
to refer to a C++ expression, whose result type is $\tau$.
...
case should it have been defined in the grammar directly.

\noindent
{\bf type-id} represents a type expression that designates any valid C++ type. 
We will be using this meta-variable in the superscript to other meta-variables 
in order to indicate a C++ type of the entity they represent.

Entities \Rule{type-id}, \Rule{statement}, \Rule{expression}, 
\Rule{constant-expression} and \Rule{identifier} refer to non-terminals from the 
C++ grammar bearing the same name. We do not elaborate them besides few comments 
on their use we make.
