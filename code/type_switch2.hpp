...
#define XTL_MATCH_PREAMBULA(s1,s2)                                              \
        struct match_uid_type {};                                               \
        enum { is_inside_case_clause = 0 };                                     \
                                                                                \
        auto&&     subject_ref1 = s1;                                           \
        auto const subject_ptr1 = addr(subject_ref1);                           \
        typedef XTL_CPP0X_TYPENAME underlying<decltype(*subject_ptr1)>::type source_type1;\
        typedef source_type1 target_type1;                                      \
        XTL_ASSERT(("Trying to match against a nullptr argument 1",subject_ptr1));\
        auto& match1 = *subject_ptr1;                                           \
        XTL_UNUSED(match1);                                                     \
                                                                                \
        auto&&     subject_ref2 = s2;                                           \
        auto const subject_ptr2 = addr(subject_ref2);                           \
        typedef XTL_CPP0X_TYPENAME underlying<decltype(*subject_ptr2)>::type source_type2;\
        typedef source_type2 target_type2;                                      \
        XTL_ASSERT(("Trying to match against a nullptr argument 2",subject_ptr2));\
        auto& match2 = *subject_ptr2;                                           \
        XTL_UNUSED(match2);
...
#define Match(s1,s2) {                                                         \
        ...
        static_assert(std::is_polymorphic<source_type2>::value, "Type of subject 2 should be polymorphic when you use Match");\
