...
{ 
    struct ClassicExprPrintVisitor : ipr::assertive_visitor_base<std::string> ///< Visitor that prints an expression in the classic C++ meaning of it.
    {
        ...
        virtual void visit(const ipr::Array_ref&        e) { result = eval_classic_ex(e.base()) + '['  + eval_classic_ex(e.member()) + ']'; } 
        virtual void visit(const ipr::Arrow&            e) 
        {
            if (e.has_impl_decl())
            {
                ...
                result = eval_classic_ex(e.base()) + "->" + eval_classic_ex(e.member()); 
        }
        virtual void visit(const ipr::Dot&              e) { result = eval_classic_ex(e.base()) + '.'   + eval_classic_ex(e.member()); }
        virtual void visit(const ipr::Arrow_star&       e) { result = eval_classic_ex(e.base()) + "->*" + eval_classic_ex(e.member()); }
        virtual void visit(const ipr::Dot_star&         e) { result = eval_classic_ex(e.base()) + ".*"  + eval_classic_ex(e.member()); }
...
cxx_printer& cxx_printer::operator<<(const ipr::Stmt& stmt)
{
    struct StatementPrintVisitor : IndentationBasedVisitor ///< Visitor that prints executable code.
    {
        ...
        virtual void visit(const ipr::Labeled_stmt&  matched) { beforeF(matched) << (is_case_label(matched.label()) ? "case " : "") << matched.label() << ": " << matched.stmt() /*, !ipr::util::view<ipr::Labeled_stmt>(matched.stmt())*/; after(matched); }
...
{ 
    struct DeclarationPrintVisitor : IndentationBasedVisitor ///< Visitor that prints definitions in the program.
    {
        ...
        virtual void visit(const ipr::Fundecl& matched)
        {
            ...
            //std::string declarator = eval_name(matched.name()) + '(' + eval_params(parameters) + ')';
            ...
            //    declarator = eval_name(matched.membership().name()) + "::" + declarator; // FIX: This won't work for general case of namespaces, specializations etc., just a quick hack
        ...
        virtual void visit(const ipr::Alias& matched)
        {
            ...
            {
                result << /*"typedef " <<*/ eval_type(*p,/*get_declarator(matched)*/eval_name(matched.name())) << ';'; // Since EDG front-end now sets up the Typedef specifier on ipr::Decl
                return;
            }
        ...
        virtual void visit(const ipr::Named_map& matched)
        {
            ...
            indent()   << prefix_specifiers(matched, ipr::Decl::FunctionSpecifier) 
                       << eval_type(matched.mapping().result_type(), eval_name(matched.name()))*/;
