...
long long display(const char* name, std::vector<long long>& timings, const LARGE_INTEGER& Freq)
{
    ...
    std::cout << name << " Time: ["
              << std::setw(4) << microseconds(min,Freq) << " -- " 
              << std::setw(4) << microseconds(avg,Freq) << "/" 
              << std::setw(4) << microseconds(med,Freq) << " -- "
              << std::setw(4) << microseconds(max,Freq) << "] Dev = " 
...
void test_randomized()
{
    ...
    QueryPerformanceFrequency(&Freq);
    //srand (ts.QuadPart/Freq.QuadPart); // Randomize pseudo random number generator
