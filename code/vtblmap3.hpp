...
private:
    ...
    struct stored_type
    {
        stored_type() : value(), hits(0), frequency(0) {}
...
//#endif
    ...
    std::ostream& operator>>(std::ostream& os) const
    {
        ...
        for (q = vtbls.begin(); q != vtbls.end(); ++q)
        {
            ...
            XTL_USE_VTBL_FREQUENCY_ONLY(<< std::setw(8) << table.find(vtbl)->second.frequency)
               << '\t';
...
#if defined(XTL_DUMP_PERFORMANCE)
    vtblmap_of(const char* fl, size_t ln, const vtbl_count_t expected_size = min_expected_size) : file(fl), line(ln), updates(0),
        cache_mask((1<<std::min(max_log_size,bit_offset_t(req_bits(expected_size-1))))-1),
        cache(cache_mask < local_cache_size ? local_cache : new cache_entry[cache_mask+1]),
        optimal_shift(VTBL_IRRELEVANT_BITS),
        table(expected_size),
...
#endif
    vtblmap_of(const vtbl_count_t expected_size = min_expected_size) : XTL_DUMP_PERFORMANCE_ONLY(file("unspecified"), line(0), updates(0),)
        cache_mask((1<<std::min(max_log_size,bit_offset_t(req_bits(expected_size-1))))-1),
        cache(cache_mask < local_cache_size ? local_cache : new cache_entry[cache_mask+1]),
        optimal_shift(VTBL_IRRELEVANT_BITS),
        table(expected_size),
    ...
    T& update(intptr_t vtbl)
    {
        ...
        if (true/*diff != m_different*/)
        {
            ...
            for (typename vtbl_to_t_map::iterator p = table.begin(); p != table.end(); ++p)
            {
                ...
                ce.vtbl = vtbl;
                ce.ptr  = &p->second;
                //XTL_USE_VTBL_FREQUENCY_ONLY(if (!p->second.frequency) p->second.frequency = get_frequency<S>(vtbl));
                //get_frequency<S>(vtbl);
            }
        }
//#if defined(XTL_DUMP_PERFORMANCE)
//        std::clog << "Vtbl:New" << std::bitset<8*sizeof(intptr_t)>((unsigned long long)vtbl) << " -> " << ((vtbl >> optimal_shift) & cache_mask) << '\t' << vtbl_typeid(vtbl).name() << std::endl;
